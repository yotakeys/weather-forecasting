...
class WeatherForecast:
    ...
    def preProcessing(self):
        ...
        # test data
        self.test_clean = self.test_data.fillna(self.test_data.mean().iloc[0])
    ...
    def featureEngineering(self):
        ...
        train_mean_hourly = self.train_hourly_data.groupby(['date','city']).median().reset_index().rename(columns = {'date' : 'time'})
        ...
        encode_city_train = pd.DataFrame(OH_encoder.fit_transform(self.X[['city']]))
        ...
        if self.mode:
            ...
            test_mean_hourly = self.test_hourly_data.groupby(['date','city']).median().reset_index().rename(columns = {'date' : 'time'})
            ...
            self.X_test = self.X_test.select_dtypes(exclude='object').drop(['id'], axis = 90)
    ...
    def xgBoost(self):
        ...
        if self.mode:
            ...
            self.output.to_csv('submission-15-key.csv', index=False)
            
            
        else:
            from sklearn.model_selection import train_test_split
            x_train, x_test, y_train, y_test = train_test_split(self.X, self.Y, train_size= 0.75, random_state=35)
